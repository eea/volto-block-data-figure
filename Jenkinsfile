            ...
            node(label: 'docker') {
              script {
                try {
                  ...
                  sh '''docker pull eeacms/volto-test; docker run -i --ipc=host --name="$BUILD_TAG-cypress" --link $BUILD_TAG-plone:plone -e NAMESPACE="$NAMESPACE" -e DEPENDENCIES="$DEPENDENCIES" -e GIT_NAME=$GIT_NAME -e GIT_BRANCH="$BRANCH_NAME" -e RAZZLE_ALLOWED_CORS_DESTINATIONS="www.eea.europa.eu,plone.org,alin.dev2aws.eea.europa.eu" -e GIT_CHANGE_ID="$CHANGE_ID" eeacms/volto-test cypress'''
                } finally {
